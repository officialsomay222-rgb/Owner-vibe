...
export const MiniPlayer = () => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div
          className="relative overflow-hidden bg-[#1a1a1a]/90 backdrop-blur-xl transform-gpu will-change-transform border border-white/5 rounded-xl p-2 flex items-center justify-between shadow-[0_4px_24px_rgba(0,0,0,0.4)] has-[.expand-btn:active]:scale-[0.98] has-[.expand-btn:focus-visible]:ring-2 has-[.expand-btn:focus-visible]:ring-white/50 transition-transform"
        >
          {/* Hidden button for expand action */}
          <button
            onClick={() => setIsExpanded(true)}
            aria-label="Expand player"
            className="absolute inset-0 w-full h-full cursor-pointer opacity-0 z-0 expand-btn"
          />
          ...
          <div className="flex items-center space-x-2 shrink-0 pr-2 relative z-10 pointer-events-none">
            <button
              onClick={(e) => {
                e.stopPropagation();
                if (!isLoadingStream) {
