...
export const MiniPlayer = () => {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div
          className="relative overflow-hidden bg-[#1a1a1a]/90 backdrop-blur-xl transform-gpu will-change-transform border border-white/5 rounded-xl p-2 flex items-center justify-between shadow-[0_4px_24px_rgba(0,0,0,0.4)] transition-transform has-[.bg-expand-btn:active]:scale-[0.98] has-[.bg-expand-btn:focus-visible]:ring-2 has-[.bg-expand-btn:focus-visible]:ring-white/50"
        >
          {/* Progress Bar Line */}
          <MiniProgressBar duration={duration} audioRef={audioRef} />

          {/* Invisible interactive background layer for expanding to avoid nested buttons */}
          <button
            aria-label="Expand player"
            className="bg-expand-btn absolute inset-0 w-full h-full cursor-pointer focus-visible:outline-none z-0"
            onClick={() => setIsExpanded(true)}
          />

          <div className="flex items-center space-x-3 overflow-hidden z-10 pointer-events-none">
          ...
          <div className="flex items-center space-x-2 shrink-0 pr-2 z-10">
            <button
              ...
              className={`p-2 transition-colors focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:outline-none rounded-full ${isLoadingStream ? 'text-white/50 cursor-not-allowed' : 'text-white hover:text-white/80'}`}
            >
              {isLoadingStream ? (
